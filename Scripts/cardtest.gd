...
func destroy() -> void:
	sub_viewport_container.use_parent_material = true
...
func handle_mouse_click(event: InputEvent) -> void:
	...
	if event.is_pressed():
		...
		tween_rot.tween_property(sub_viewport_container.material, "shader_parameter/x_rot", 0.0, 0.1)
		tween_rot.tween_property(sub_viewport_container.material, "shader_parameter/y_rot", 0.0, 0.1)
	else:
		# drop card
...
func _on_mouse_entered() -> void:
	...
	tween_hover.tween_property(self, "scale", Vector2(1.1, 1.1), 1)
...
func _on_mouse_exited() -> void:
	...
	if tween_hover and tween_hover.is_running():
		tween_hover.kill()
	tween_hover = create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
	tween_hover.tween_property(self, "scale", Vector2.ONE, 1.0)
